...
// var namea = '';
// var childa = exec('a.exe');
// childa.stdin.write('INFO 1.0\n');
// childa.stdout.on('data', async function(data) {
//     await aName = data.toString();
...
// childa.stdout.on('data', data => aName = data.toString());

// console.log(aName);
childa.stdin.write('INFO 1.0\n');
const getAName = new Promise((resolve, reject) => {
    return childa.stdout.on('data', data => data);
});

getAName
    .then(function(data) {
        aName = data.toString();
        console.log('then' + aName);
    });
console.log('all' + aName);
